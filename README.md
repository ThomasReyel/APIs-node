...
- Características:
    ...
    5. Representação de dados: Rest geralmente retorna o dados ou em json ou xml, porém jsom
	é mais popular por ser mais compatível com Node
...
## Introdução a express.js
    ...
    • Características do Express.js
        ◦ Minimalista: O Express fornece apenas o necessário para o desenvolvedor, permitindo que a aplicação seja mais flexível e consuma menos dados
        ◦ Modularidade com Middlewares: Middlewares permitem adicionar lógica no processo das requisições como um (meio do caminho) ele pode ser usado no tratamento de erros ou em autenticação de informações.
          
        ◦ Flexibilidade de Roteamento:  Express fornece um sistema de roteamento poderoso e flexível, que permite associar URLs específicas a funções de tratamento. É possível definir rotas para diferentes métodos HTTP (como GET, POST, PUT e DELETE) e para URLs dinâmicas que aceitam parâmetros.
        ◦ Não opinativo: permite que você organize as pastas da forma que você preferir, sem ter uma estrutura rígida
        ◦ 
